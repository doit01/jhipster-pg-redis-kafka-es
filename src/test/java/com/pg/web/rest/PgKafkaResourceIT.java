...
import com.pg.IntegrationTest;
import com.pg.config.EmbeddedKafka;
import com.pg.config.KafkaSseConsumer;
import com.pg.config.KafkaSseProducer;
...
class PgKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pg-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pg-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/pg-kafka/unregister"));
                return;
            }
        }
